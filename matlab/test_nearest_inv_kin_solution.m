...
adjusted_trajectory = zeros(3, numel(times));
for i = 1:numel(times)
    trajectory(:, i) = getCircularTrajectory(circle_orientation, origin, radius, times(i), period);
end

for t=1:numel(times)
    target_pose(1:3,4) = trajectory(1:3,t);
    try
        joint_pos(1:6,1) = getInverseKinematics(robot,target_pose,last_joint_pos);
    catch
        [delta_p, fval] = fmincon(@(x)objective(x), ...
            zeros(3,1), [], [], [], [], ...
            [-1, -1, -1], [1, 1, 1], ...
            @(x)constraint(x, target_pose, last_joint_pos), ...
            options);
        target_pose = target_pose + [zeros(3),delta_p;zeros(1,4)];
        joint_pos(1:6,1) = getInverseKinematics(robot,target_pose,last_joint_pos);
    end
    adjusted_trajectory(1:3,t) = target_pose(1:3,4);
...
plot3(adjusted_trajectory(1, :), adjusted_trajectory(2, :), adjusted_trajectory(3, :), 'g', 'LineWidth', 2);
...
function obj = objective(x)
    % x 是位置调整量 [Δx, Δy, Δz]
    obj = x(1)^2 + x(2)^2 + x(3)^2; % 最小化调整量的欧几里得范数
end

function [c, ceq] = constraint(x, target_pose, ref_joint_pos)
    % x 是位置调整量 [Δx, Δy, Δz]
    adjusted_pose = target_pose + [zeros(3),x;zeros(1,4)];
    
    % 调用逆运动学求解器
    try
        theta = getInverseKinematics(robot,adjusted_pose,ref_joint_pos);
