/**
 * @file arm_interface.h
 * @author pansamic (pansamic@foxmail.com)
 * @brief AgileX Piper robotic arm communication and packet parser.
 * @version 0.1
 * @date 2025-05-05
 * 
 * @copyright Copyright (c) 2025
 * 
 */
#ifndef __ARM_INTERFACE_H__
#define __ARM_INTERFACE_H__

#include <thread>
#include <mutex>
#include <atomic>
#include <linux/can.h>
#include <linux/can/raw.h>
#include <sys/socket.h>
#include <sys/ioctl.h>
#include <net/if.h>
#include <Eigen/Core>
#include <Eigen/Geometry>

class ArmInterface
{
public:

    typedef enum ArmControlMode_t
    {
        MODE_STANDBY = 0,
        MODE_CAN,
        MODE_TEACH,
        MODE_ETHERNET,
        MODE_WIFI,
        MODE_REMOTE,
        MODE_DUAL_TEACH,
        MODE_OFFLINE_TRAJECTORY
    }ArmControlMode;

    typedef enum ArmStatus_t
    {
        STATUS_NORMAL,
        STATUS_EMERGENCY_STOP,
        STATUS_NO_SOLUTION,
        STATUS_SIGULAR,
        STATUS_OVER_POSITION_LIMIT,
        STATUS_JOINT_COMMUNICATION_ERROR,
        STATUS_JOINT_BRAKE_UNRELEASED,
        STATUS_SELF_COLLISION,
        STATUS_TEACH_OVERSPEED,
        STATUS_JOINT_EXCEPTION,
        STATUS_OTHER_EXCEPTION,
        STATUS_TEACH_RECORD,
        STATUS_TEACH_EXECUTE,
        STATUS_TEACH_SUSPEND,
        STATUS_NTC_OVER_TEMPERATURE,
        STATUS_RELEASE_RES_OVER_TEMPERATURE
    }ArmStatus;

    typedef enum MoveMode_t
    {
        MOVE_MODE_POSITION = 0,
        MOVE_MODE_JOINT,
        MOVE_MODE_LINEAR,
        MOVE_MODE_CIRCULAR,
        MOVE_MODE_MIT
    }MoveMode;

    typedef enum TeachStatus_t
    {
        TEACH_STATE_OFF = 0,
        TEACH_STATE_START,
        TEACH_STATE_END,
        TEACH_STATE_EXECUTE,
        TEACH_STATE_SUSPEND,
        TEACH_STATE_RESUME,
        TEACH_STATE_TERMINATE,
        TEACH_STATE_TRAJ_BEGIN
    }TeachStatus;

    typedef enum MotionStatus_t
    {
        MOTION_REACH = 0,
        MOTION_NOT_REACH
    }MotionStatus;

    typedef enum ActuatorStatus_t
    {
        ACTUATOR_STATUS_LOW_VOLTAGE = 1<<0,
        ACTUATOR_STATUS_MOTOR_OVER_TEMPERATURE = 1<<1,
        ACTUATOR_STATUS_OVER_CURRENT = 1<<2,
        ACTUATOR_STATUS_OVER_TEMPERATURE = 1<<3,
        ACTUATOR_STATUS_COLLISION_PROTECT = 1<<4,
        ACTUATOR_STATUS_ERROR = 1<<5,
        ACTUATOR_STATUS_DISABLED = 1<<6,
        ACTUATOR_STATUS_LOCKED_ROTOR = 1<<7
    }ActuatorStatus;

    ArmInterface(const char* can_interface);
    ~ArmInterface();

    Eigen::Vector<double,6> getActuatorsPosition() const
    {
        return joint_pos_;
    }
    Eigen::Vector<double,6> getActuatorsVelocity() const
    {
        return joint_vel_;
    }
    Eigen::Vector<double,6> getActuatorsTorque() const
    {
        return joint_torque_;
    }
    void parseCanFrame(struct can_frame* can_frame);

    /**
     * @brief Send MIT protocol command to piper manipulator.
     * 
     * @param actuator_id from 0 to 5.
     * @param position position without joint limit bound. unit: rad.
     * @param velocity velocity without joint velocity bound. unit: rad/s.
     * @param kp 
     * @param kd 
     * @param torque 
     */
    void sendControlMessage(
        const uint8_t actuator_id,
        const int16_t position,
        const int16_t velocity,
        const int16_t kp,
        const int16_t kd,
        const int8_t torque) const;

    void setMITMode() const;
    void setJointPositionMode() const;
    void setEndEffectorPoseMode() const;

    void enableActuators() const;
    void disableActuators() const;

    void setActuatorControl(
        const Eigen::Vector<double,6>& joint_pos,
        const Eigen::Vector<double,6>& joint_vel,
        const Eigen::Vector<double,6>& joint_torque) const;

    void setActuatorTorque(const Eigen::Vector<double,6>& joint_torque) const;

    void setActuatorPosition(const Eigen::Vector<double,6>& joint_pos) const;
    
    void setEndEffectorPose(const Eigen::Vector3d& pos, const Eigen::Quaterniond& orientation) const;

    void setGripperCurrentAsZero() const;
    void setGripperControl(const double position, const double torque) const;

private:
    int running_;
    /* CAN socket id */
    int can_socket_;

    /* CAN socket address */
    struct sockaddr_can can_addr_;
    struct ifreq can_ifr_;

    /* thread for CAN polling receiption */
    std::thread read_thread_;

    /* CAN socket mutex */
    std::mutex mtx_;

    Eigen::Vector<double,6> joint_pos_;
    Eigen::Vector<double,6> joint_vel_;
    Eigen::Vector<double,6> joint_torque_;

    std::array<float,6> actuator_voltage_;
    std::array<int16_t,6> actuator_temperature_;
    std::array<int8_t,6> motor_temperature_;
    std::array<uint8_t,6> actuator_status_;

    void can_receive_thread();
};

#endif // __ARM_INTERFACE_H__