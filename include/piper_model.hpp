...
// --- Configuration ---
constexpr std::size_t PiperArmNumDof = 9;
...
const std::array<std::size_t, PiperArmNumDof> piper_parent_index =
{{
 ...
 * @return RigidBodyTree<T, PiperArmNumDof>::Builder A builder instance for the Piper model.
 */
template<typename T>
constexpr typename RigidBodyTree<T, PiperArmNumDof>::Builder
getPiperArmModelBuilder(const std::array<T, 3>& base_position, const std::array<T, 3>& base_orientation)
{
    typename RigidBodyTree<T, PiperArmNumDof>::Builder builder{};
    ...
    for (std::size_t child_idx = 0; child_idx < piper_parent_index.size(); ++child_idx)
    {
        ...
        if (parent_id > 0) { // If it has a parent (not the root)
            ...
            if (parent_idx < PiperArmNumDof) // Safety check
            {
            ...
            // where parent_idx >= NumDof, indicating invalid parent_index data.
        }
    }
