...
def parse_joint_positions(log_path):
    ...
    # Regex patterns
    timestamp_pattern = r"\[(.*?)\]"
    left_pattern = r"Left arm IK solution:([-\d., ]+)"
    right_pattern = r"Right arm IK solution:([-\d., ]+)"
    ...
    with open(log_path, 'r') as file:
        for line in file:
            ...
            if "Left arm IK solution:" in line:
            ...
            elif "Right arm IK solution:" in line:
...
def plot_dual_arm_joint_positions(left_timestamps, left_data, right_timestamps, right_data):
    ...
    fig.suptitle("Dual Arm IK Solution")
